...
cryptonight_ctx* minethd_alloc_ctx()
{
	...
	case jconf::never_use:
		...
		if (ctx == NULL)
			printer::inst()->print_msg(L0, "", msg.warning);
		return ctx;
...
std::vector<minethd*>* minethd::thread_starter(miner_work& pWork)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if(cfg.iCpuAff >= 0)
			printer::inst()->print_msg(L1, "", cfg.bDoubleMode ? "double" : "single", (int)cfg.iCpuAff);
		else
			printer::inst()->print_msg(L1, "", cfg.bDoubleMode ? "double" : "single");
	}
